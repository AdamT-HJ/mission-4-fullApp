...
// allows use of await at 'top level' without being inside a function
//====IIFE STARTS HERE AND WRAPS BASICALLY EVERYTHING for main server setup and startup====================================================//
(async () => {
    ...
    // commented out due to docker secrets use
    ...
    // api key using docker secrets
    // docker mounts secrets at /run/secrets/"secret_name"
    // this way server will work if in local environment or container
    
    // API KEY LOADING
    const API_KEY_SECRET_PATH="/run/secrets/api_key";
    let apiKey;

    // so server can run locally or in container, because using docker secrets
    // looks of secret (container) then looks for env key (local)
    async function loadApiKey() {
        try{
            //first looks in docker secret file (container env)
        apiKey = (await fs.readFile(API_KEY_SECRET_PATH, 'utf8')).trim();
        console.log("API KEY loaded from Docker secret.");
        }
        catch (error){
            // secret not found, tries process.env (local env) also setup by dotenv.config() at start of this file
        apiKey = process.env.API_KEY;
        if(!apiKey){
            console.error("API_KEY not set. Please provide it via Docker secret (secret_api_key.txt) or env var (.env for local dev).");
            process.exit(1);
        }
            
        console.log("API KEY loaded from env variable"); 
        }
    
    }

    // Call loadAPiKey and AWAIT. so can use in const ai
    await loadApiKey();
    ...
    // call and AWAIT for setup
    await createOrCheckForSessionsDirectory();
    ...
    // --------- Start Server------//
    //ensuring API key is loaded before server starts
